...
from config import NAVER_ACCESS_KEY, NAVER_SECRET_KEY
...
def rag(company_name, company_type, bucket_name, doc_name):
    ...
    queries = []
    queries = extract_queries(query_response.content)
    ...
    compressor_response = model([
        SystemMessage(content="You are an Okr AI assistant."),
        HumanMessage(content=compressor_prompt)
    ])
    
    # print("Compressor Response:", compressor_response.text)

    # 7. JSON 형식 반환 및 예외 처리
    description_data = parse_description(compressor_response.content)
